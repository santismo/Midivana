...
struct ContentView: View {
  ...
  var body: some View {
    ZStack(alignment: .topTrailing) {
      ...
      if showingMacKeys {
        MacKeyWindow(app: app, visible: $showingMacKeys)
          .padding(.top, 90)
          .padding(.trailing, 18)
...
private struct MacKeyWindow: View {
  ...
  @State private var dragStart: CGSize?
  @State private var size = CGSize(width: 220, height: 204)
  ...
  var body: some View {
    ZStack(alignment: .bottomTrailing) {
      VStack(spacing: keyGap) {
        ...
        key("↑", cc: app.preset.keyPadCCUp)
        HStack(spacing: keyGap) {
          key("←", cc: app.preset.keyPadCCLeft)
          key("i", cc: app.preset.keyPadCCI)
          key("→", cc: app.preset.keyPadCCRight)
        }
        key("↓", cc: app.preset.keyPadCCDown)
  ...
  private var keySize: CGFloat {
    min(max(50, min((size.width - 34) / 3.0, (size.height - 52) / 3.05)), 116)
  }

  private var keyGap: CGFloat {
    max(5, keySize * 0.14)
  }
  ...
  private var resizeGesture: some Gesture {
    DragGesture(minimumDistance: 0, coordinateSpace: .named("MidivanaRoot"))
      .onChanged { value in
        ...
        size = CGSize(
          width: min(max(176, resizeStart.width + value.translation.width), 420),
          height: min(max(164, resizeStart.height + value.translation.height), 390)
        )
      }
      .onEnded { _ in
        resizeStart = nil
  ...
  private func key(_ label: String, cc: Int) -> some View {
    KeyPadButton(label: label, size: keySize, theme: app.preset.theme) { pressed in
      app.setKeyPadCC(number: cc, pressed: pressed)
    }
  }
}
...
private struct FretMarkers: View {
  ...
  var body: some View {
    ...
    .padding(.vertical, max(0, CGFloat(app.preset.theme.markerOffset) / 28))
    .offset(y: CGFloat(app.preset.theme.markerOffset))
